...
- **Android** (`android/`, phase 1): Gradle app embedding a self-built static CRuby (`android/scripts/build-runtime.sh`); it runs a probe, not yet the core. Open native libraries by name, never by path (they stay inside the APK). `docs/android.md` has the build, the findings and the next steps.
