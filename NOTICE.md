...
- **Start-up flow:** fork notice at the licence agreement and in the welcome wizard; EltenLink-only steps (launcher stamp, SMS two-factor authentication, premium and donation pages, EltenLink links) removed or disabled. The built-in updater talks to the Klango server only (installer names `KlangtenSetup.exe`, `Klangten.pkg`, `klangten-linux.run`, architecture sent with update requests, installer URL restricted to the configured API) and can be disabled with `KLANGTEN_UPDATES=0`.
