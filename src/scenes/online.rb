...
class Scene_Online
  def main
    ...
    begin
      ...
      Log.warning("Online list failed: #{e.message}")
      @onl = []
      alert(_("Error"))
    end
                selt = []
    ...
    cnt=@onl.size
    @sel = ListBox.new(selt,header: (np_("Online", "%{count} user online", "%{count} users online", cnt))%{:count=>cnt}, index: 0, flags: 0, quiet: false)
    apply_user_status_states(@sel, @onl, false)
